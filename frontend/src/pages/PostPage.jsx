import { Box, Divider, Heading, IconButton, VStack, Spinner, Center, Text } from "@chakra-ui/react";
...
const PostPage = () => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${API_URL}/replies/${mainPostData.id}`);
  ...
  const handleCommentSubmit = async (newCommentText) => {
    ...
    if (!newCommentText.trim() || !user || !mainPostData?.id) {
      console.warn("コメント内容がないか、ログインしていないか、元投稿がありません。");
      return;
    }
    ...
    try {
      ...
      const newCommentForState = {
        ...
        likes: [],
        user: { // ユーザー情報を付与（Postコンポーネントが表示に使うため）
      ...
      alert(`コメントの投稿に失敗しました: ${error.response?.data?.detail || error.message}`);
    }
  };
  ...
  return (
    <VStack spacing={0} align="stretch">
      ...
      {mainPostData ? (
        ...
        <Center h="20vh"><Text color="gray.500">投稿データが見つかりません。</Text></Center>
      )}
      ...
      <Box p={4}>
        ...
        {isLoadingComments ? (
          <Center><Spinner color="orange.400" /></Center>
        ) : (
