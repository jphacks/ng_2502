  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        const data = await response.json();
        const allPosts = Array.isArray(data) ? (data as PostItem[]) : [];
        const privateOnlyPosts = allPosts.filter(
          (post) => post.isToFollower === false,
        );

        console.log("✅ private投稿を取得:", privateOnlyPosts);
        setPosts(privateOnlyPosts);
      } catch (error) {
    ...
    <YStack space="$0" flex={1}>
      ...
      {posts.length === 0 ? (
        <YStack flex={1} justifyContent="center" alignItems="center" py="$6">
          <Text color="$gray8">private投稿はまだありません。</Text>
        </YStack>
      ) : (
        <ScrollView
          style={{ flex: 1 }}
